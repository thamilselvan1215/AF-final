...
/* ──────────────────────────────────────────────
   ...
   ────────────────────────────────────────────── */
const SUPABASE_URL = 'https://mxbkwpoehnhhwglfdmgp.supabase.co';
const SUPABASE_KEY = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6Im14Ymt3cG9laG5oaHdnbGZkbWdwIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NzMxMjE3MDEsImV4cCI6MjA4ODY5NzcwMX0.pvst5o2_blNN28v3KYXINQVlUbBwWKyhTkPckBfwOE8';
...
function validateForm(formEl) {
    ...
    formEl.querySelectorAll('[required]').forEach(field => {
        ...
        // Skip hidden inputs (type=hidden) and visually hidden fields
        // (e.g. "other school" inputs hidden inside display:none wrappers)
        if (field.type === 'hidden') return;
        if (field.offsetParent === null) return;   // element or ancestor is display:none
    ...
    // mobile validation (use dynamic IDs, not #mobile)
    formEl.querySelectorAll('input[type="tel"]').forEach(mob => {
        if (mob.value && !/^\d{10}$/.test(mob.value.trim())) {
    ...
    // email
    formEl.querySelectorAll('input[type="email"]').forEach(em => {
...
async function initAdmin() {
    ...
    document.getElementById('filter-gender')?.addEventListener('change', () => renderTable(currentTable));
    document.getElementById('filter-community')?.addEventListener('change', () => renderTable(currentTable));
    document.getElementById('filter-min-perc')?.addEventListener('input', () => renderTable(currentTable));
    document.getElementById('filter-max-perc')?.addEventListener('input', () => renderTable(currentTable));
    document.getElementById('filter-region')?.addEventListener('input', () => renderTable(currentTable));
    document.getElementById('search-input')?.addEventListener('input', () => renderTable(currentTable));
    
    // Clear Filters
    document.getElementById('clear-filters-btn')?.addEventListener('click', () => {
        ['filter-dept', 'filter-gender', 'filter-community', 'filter-min-perc', 'filter-max-perc', 'filter-region', 'search-input'].forEach(id => {
            const el = document.getElementById(id);
            if (el) el.value = '';
        });
        renderTable(currentTable);
...
async function loadAllData() {
    try {
        const [ug, pg, le, ug2] = await Promise.all([
            db.fetchAll('ug_admissions').catch(() => []),
            db.fetchAll('pg_admissions').catch(() => []),
            db.fetchAll('lateral_admissions').catch(() => []),
            db.fetchAll('ug2_admissions').catch(() => [])
        ]);
        
        // Keep them separate so the tabs work cleanly
        adminData = { ug, ug2, pg, le };
...
function updateStats() {
    const total = adminData.ug.length + adminData.ug2.length + adminData.pg.length + adminData.le.length;
    setText('stat-total', total);
    setText('stat-ug', adminData.ug.length);
    setText('stat-ug2', adminData.ug2.length);
...
function renderTable(type) {
    ...
    const regionText = document.getElementById('filter-region')?.value?.toLowerCase();
    ...
    // 3. Community Filter
    if (communityFilter) {
        rows = rows.filter(r => (r.community || '') === communityFilter);
    }
    
    // 4. Region/City Filter (checks address and school)
    if (regionText) {
        rows = rows.filter(r => {
            const addr = (r.address || '').toLowerCase();
            const sslcSchool = (r.sslc_school || '').toLowerCase();
            const hscSchool = (r.hsc_school || '').toLowerCase();
            return addr.includes(regionText) || sslcSchool.includes(regionText) || hscSchool.includes(regionText);
        });
    }
    
    // 5. Percentage Range Filter
    if (!isNaN(minPerc) || !isNaN(maxPerc)) {
        rows = rows.filter(r => {
            // Find the main percentage for this student based on admission type
            let percStr = r.hsc_percentage || r.degree_percentage || r.diploma_percentage || '';
            let val = parseFloat(percStr);
            if (isNaN(val)) return false; // If they don't have a valid percentage, exclude them
            
            if (!isNaN(minPerc) && val < minPerc) return false;
            if (!isNaN(maxPerc) && val > maxPerc) return false;
            return true;
        });
    }
    ...
    if (search) {
        rows = rows.filter(r =>
            ...
            (r.mobile || r.student_mobile || r.parent_mobile || '').includes(search) ||
...
function exportExcel() {
    ...
    ['ug', 'ug2', 'pg', 'le'].forEach(key => {
        if (adminData[key] && adminData[key].length > 0) {
            const ws = XLSX.utils.json_to_sheet(adminData[key]);
            let sheetName = '';
            if (key === 'ug') sheetName = 'UG Admissions';
            else if (key === 'ug2') sheetName = 'UG Phase 2';
            else if (key === 'pg') sheetName = 'PG Admissions';
            else sheetName = 'Lateral Entry';
